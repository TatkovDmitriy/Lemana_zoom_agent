...
- Транскрипцию делает self-hosted faster-whisper (модель large-v3, CPU/int8).
- Интеграцию с реальным Zoom Linux SDK выносим в отдельный тикет.

Следующие шаги:
- Мария: создать тикет на binary integration.
- Алексей: проверить RAM на Railway-плане (large-v3 требует ~4 GB).`;
...
export async function transcribeAudio(audioFilePath: string): Promise<string> {
  ...
  const { stdout } = await execFileAsync(
    ...
    {
      // large-v3 on CPU can use ~4 GB; raise stdout buffer accordingly.
